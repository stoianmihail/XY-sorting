...
// Construction of spline taken from:
...
namespace util {
   ...
   // print the errors of the spline to the cdf
   {
      ...
      std::cout << "cdf_size: " << cdf.size() << std::endl << "spline_size: " << spline.size() << std::endl << "Errors for spline - (average, max): (" << errs.first << ", " << errs.second << ")" << std::endl << std::endl;  
   }
   ...
   template<class KeyCoord>
   static std::vector<Coord> buildCdf(const std::vector<KeyCoord> &file)
   ...
   // assumes the function is already sorted after key
   {
      ...
      {
         unsigned pos = 0;
         uint32_t last = file.front().first;
         for (auto d : file) {
            if (d.first != last) {
